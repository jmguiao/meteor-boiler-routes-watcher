...
export default class Watcher {
  constructor(parent) {
    ...
    this.parent = parent;
    this.callFunction = this.parent ? this.parent.callFunction : () => {};
    this.callSubscribe = this.parent ? this.parent.callSubscribe : () => {};
    this._subscriptions = {};
    this.mounted = {};
    this.isRendering_ = {};
    this.stack = {};
  ...
  removeWatcher(name) {
    ...
    else console.warn("Removing watcher but no attached component!");
  }
  /**
  ...
  callFunc(...args) {
    ...
    if (this.callFunction) {
      for (let x = 1; x < args.length; x++) args[x] = traverse(args[x]);
      ...
      for (let key in data) {
        ...
        if (this._subscriptions[key]) {
          ...
          if (
            ...
            Meteor.default_connection._subscriptions[lastId]
          )
        ...
        isReady = this._subscriptions[key].ready() && isReady;
        if (
          this._subscriptions[key].subscriptionId != lastId &&
          Meteor.default_connection &&
          Meteor.default_connection._subscriptions[lastId] &&
